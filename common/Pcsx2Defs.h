...
/* make sure __POSIX__ is defined for all systems 
 * where we assume POSIX compliance */
...
/* The C++ standard doesn't allow `offsetof` to be used on non-constant values 
 * (e.g. `offsetof(class, field[i])`)
 ...
 *   EnumToString(value);
 */
#define ImplementEnumOperators(enumName) extern const char* EnumToString(enumName id)
