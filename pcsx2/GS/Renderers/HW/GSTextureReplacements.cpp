...
namespace GSTextureReplacements
{
	...
	static slock_t* s_replacement_texture_cache_mutex(void)
	{
...
static void WorkerThreadEntryTrampoline(void* arg)
{
	...
	static scond_t* s_worker_thread_cv(void)
	{
		static scond_t* cond = scond_new();
		return cond;
	}
	...
	static scond_t* s_worker_thread_done_cv(void)
	{
		static scond_t* cond = scond_new();
		return cond;
	}
...
void GSTextureReplacements::ReloadReplacementMap()
{
	...
	// clear out the caches
	{
		...
		SLockGuard lock(s_replacement_texture_cache_mutex());
	...
	if (!s_replacement_texture_filenames.empty())
	{
		...
		{
			log_cb(RETRO_LOG_WARN, "Replacement textures were found, and GPU palette conversion is enabled without full preloading.\n");
			log_cb(RETRO_LOG_WARN, "Palette textures will be disabled. Please enable full preloading or disable GPU palette conversion.\n");
		}
	}
}
...
std::optional<GSTextureReplacements::ReplacementTexture> GSTextureReplacements::LoadReplacementTexture(const TextureName& name, const std::string& filename, bool only_base_image)
{
	...
	{
		log_cb(RETRO_LOG_WARN, "Failed to load replacement texture %s\n", filename.c_str());
		return std::nullopt;
	}
...
void GSTextureReplacements::WorkerThreadEntryPoint()
{
	...
	while (s_worker_thread_running)
	{
		...
		{
			scond_wait(s_worker_thread_cv(), s_worker_thread_mutex());
			continue;
		}
		...
		if (s_worker_thread_queue.empty())
			scond_broadcast(s_worker_thread_done_cv());
		lock.Unlock();
		fn();
		lock.Lock();
	}
...
void GSTextureReplacements::SyncWorkerThread()
{
	...
	while (!s_worker_thread_queue.empty() && s_worker_thread_running)
		scond_wait(s_worker_thread_done_cv(), s_worker_thread_mutex());
}
