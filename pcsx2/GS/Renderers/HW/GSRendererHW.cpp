...
{
	if (m_mv && m_mv(*this)) // Handled by HW hack.
		return;
...
GSTextureCache::RenderTarget, rt_end_bp)) == nullptr ||
	...
	if (!no_rt)
	{
		...
		rt = g_texture_cache->LookupTarget(FRAME_TEX0, t_size, target_scale, GSTextureCache::RenderTarget, true,
			fm, false, force_preload,
			preserve_rt_color, m_r);
...
bool GSRendererHW::CanUseTexIsFB(const GSTextureCache::Target* rt, const GSTextureCache::Source* tex, const TextureMinMaxResult& tmm) const
{
...
GSRendererHW::CLUTDrawTestResult GSRendererHW::PossibleCLUTDraw()
{
	...
	if (PRIM->TME)
	{
		...
		if (GSConfig.UserHacks_GPUTargetCLUTMode != GSGPUTargetCLUTMode::Disabled)
		{
			...
			GSTextureCache::Target* tgt = g_texture_cache->GetExactTarget(m_cached_ctx.TEX0.TBP0, m_cached_ctx.TEX0.TBW, GSTextureCache::RenderTarget, m_cached_ctx.TEX0.TBP0);
			if (tgt)
			{
				bool is_dirty = false;
	...
	if (format == 0)
	{
		...
		for (int y = top; y < bottom; y++)
		{
			...
			for (int x = left; x < right; x++)
				*pa.value(x) = vert_color; // Here the constant color
		}
	}
	else if (format == 1)
	{
