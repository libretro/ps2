...
GSTextureCache::Source* GSTextureCache::LookupSource(const GIFRegTEX0& TEX0, const GIFRegTEXA& TEXA, const GIFRegCLAMP& CLAMP, const GSVector4i& r, const GSVector2i* lod, bool possible_shuffle)
{
	const GSLocalMemory::psm_t& psm_s = GSLocalMemory::m_psm[TEX0.PSM];
...
bool GSTextureCache::Move(u32 SBP, u32 SBW, u32 SPSM, int sx, int sy, u32 DBP, u32 DBW, u32 DPSM, int dx, int dy, int w, int h)
{
	...
	{
		GSTexture::Format fmt     = src->m_texture->GetFormat();
		const GSVector4i src_size = GSVector4i(src->m_texture->GetSize()).xyxy();
		GSTexture* tmp_texture    = src->m_texture->IsDepthStencil() ?
				g_gs_device->CreateDepthStencil(src_size.x, src_size.y, fmt, false) :
				g_gs_device->CreateRenderTarget(src_size.x, src_size.y, fmt, false);
