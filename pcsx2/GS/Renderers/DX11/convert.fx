...
"}\n"
"\n"
"uint move_word(int2 p)\n"
"{\n"
"	uint4 c = uint4(Texture.Load(int3(p, 0)) * 255.0f + 0.5f);\n"
"	return c.r | (c.g << 8) | (c.b << 16) | (c.a << 24);\n"
"}\n"
"\n"
"int2 move_xy(uint v)\n"
"{\n"
"	return int2(int(v & 0xFFFFu), int(v >> 16));\n"
"}\n"
"\n"
"PS_OUTPUT ps_move_texels(PS_INPUT input)\n"
"{\n"
"	// See the GLSL version; the constants arrive in the merge layout,\n"
"	// BGColor as the four pairs and EMODA as the flags.\n"
"	uint4 MoveA = asuint(BGColor);\n"
"	uint flags = asuint(EMODA);\n"
"	int scale = int(flags & 0xFFu);\n"
"	uint nibbles = (flags >> 8) & 0xFFu;\n"
"	int entries = int((flags >> 16) & 0xFFu);\n"
"	bool has_old = ((flags >> 24) & 1u) != 0u;\n"
"	int2 rel = int2(input.p.xy) - move_xy(MoveA.x) * scale;\n"
"	int2 native = rel / scale;\n"
"	int2 sub = rel - native * scale;\n"
"	uint word = has_old ? move_word(move_xy(MoveA.y) + rel) : 0u;\n"
"	uint mask = (nibbles >= 8u) ? 0xFFFFFFFFu : ((1u << (nibbles * 4u)) - 1u);\n"
"	for (int e = 0; e < entries; e++)\n"
"	{\n"
"		uint ent = move_word(move_xy(MoveA.w) + int2(native.x * entries + e, native.y));\n"
"		if ((ent & 0x80000000u) == 0u)\n"
"			continue;\n"
"		int2 sp = int2(int(ent & 0x1FFFu), int((ent >> 13) & 0x1FFFu));\n"
"		uint v = (move_word(move_xy(MoveA.z) + sp * scale + sub) >> (((ent >> 26) & 7u) * 4u)) & mask;\n"
"		uint shift = uint(e) * nibbles * 4u;\n"
"		word = (word & ~(mask << shift)) | (v << shift);\n"
"	}\n"
"	PS_OUTPUT output;\n"
"	output.c = float4(uint4(word & 0xFFu, (word >> 8) & 0xFFu, (word >> 16) & 0xFFu, word >> 24)) / 255.0f;\n"
"	return output;\n"
"}\n"
"\n"
...
"	uint2 sub = uint2(0u, 0u);\n"
"	uint2 iscale = uint2(1u, 1u);\n"
"	if (floor(ScaleFactor) == ScaleFactor && ScaleFactor > 1.0f)\n"
"	{\n"
"		iscale = uint2(ScaleFactor, ScaleFactor);\n"
"		sub = pos % iscale;\n"
"		pos /= iscale;\n"
"	}\n"
"\n"
...
"	uint block_num = (block_xy.y * (DBW / 128)) + block_xy.x + uint(DOFFSET);\n"
...
"	uint n = max(uint(BGColor.y), 1u);\n"
"	uint2 sub = uint2(uint(input.p.x) % n, uint(input.p.y));\n"
"	uint index = uint(input.p.x) / n + uint(DOFFSET);\n"
"	uint subgroup = (index / 8u) % 4u;\n"
"	uint2 pos;\n"
"	pos.x = (index % 8u) + ((subgroup >= 2u) ? 8u : 0u);\n"
"	pos.y = ((index / 32u) * 2u) + (subgroup % 2u);\n"
"\n"
"	int2 final = (n > 1u) ? int2((offset + pos) * n + sub) : int2(floor(float2(offset + pos) * scale));\n"
"	PS_OUTPUT output;\n"
...
"	uint n = max(uint(BGColor.y), 1u);\n"
"	uint2 sub = uint2(uint(input.p.x) % n, uint(input.p.y));\n"
"	uint clut_x = uint(input.p.x) / n;\n"
"	uint index = (min((clut_x >> 4u) + (uint(DOFFSET) >> 4u), 15u) << 4u) | (clut_x & 15u);\n"
"\n"
