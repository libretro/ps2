...
"	float2 STScale;\n"
"	float4 ChannelOffset;\n"
"	float4x4 DitherMatrix;\n"
"	float ScaledScaleFactor;\n"
"	float RcpScaleFactor;\n"
"	float RtScaleFactor;\n"
"	// Fragments the draw's native pixel grid starts at: half a native pixel\n"
"	// under the half-pixel offset that aligns the draw to native.\n"
"	float NativeOffset;\n"
"};\n"
"\n"
...
"}\n"
"\n"
"// The fragment's position, for the palette's sample choice (set by ps_main).\n"
"static float2 s_frag;\n"
"\n"
"float4 sample_p(uint u)\n"
"{\n"
"	// A palette read from a target drawn at an integer scale keeps the\n"
"	// target's samples (GSClut::Read32): an entry is as many texels wide as\n"
"	// the palette has rows. The fragment's place inside its native pixel\n"
"	// picks the sample, as PS_SAMPLE_MAP picks a texel's; a palette at\n"
"	// native size has only the one.\n"
"	uint pw, ph;\n"
"	Palette.GetDimensions(pw, ph);\n"
"	int n = int(ph);\n"
"	int2 sub = int2(0, 0);\n"
"	if (n > 1)\n"
"	{\n"
"		float s = RtScaleFactor;\n"
"		float2 start = floor((s_frag - NativeOffset) / s) * s + NativeOffset;\n"
"		sub = min(int2((s_frag - start) * float(n) / s), int2(n - 1, n - 1));\n"
"	}\n"
"	return Palette.Load(int3(int(u) * n + sub.x, sub.y, 0));\n"
"}\n"
"\n"
...
"		t = sample_p(uint((depth >> 8) & 0xFF)) * 255.0f;\n"
"	}\n"
...
"		t = sample_p(uint(depth & 0xFF)) * 255.0f;\n"
"\n"
"		// Msb is easier\n"
...
"}\n"
"\n"
"float4 sample_color(float2 st, float uv_w, float2 frag)\n"
"{\n"
"	#if PS_TCOFFSETHACK\n"
"	st += TC_OffsetHack.xy;\n"
"	#endif\n"
...
"	float2 dd;\n"
"\n"
"	if (PS_LTF == 0 && PS_AEM_FMT == FMT_32 && PS_PAL_FMT == 0 && PS_REGION_RECT == 0 && PS_WMS < 2 && PS_WMT < 2 && PS_SAMPLE_MAP == 0)\n"
"	{\n"
...
"		{\n"
"#if PS_NATIVE_TAPS\n"
"			// The native draw blends the two texels around each pixel's coordinate;\n"
"			// drawn at scale, the taps stay a native texel apart and keep the pixel's\n"
"			// weights, taken where the pixel's first fragment sits, while each\n"
"			// fragment reads its own samples of those texels: a copy stays a copy, a\n"
"			// blur blurs as it does natively.\n"
"			float s = RtScaleFactor;\n"
"			float ts = 1.0f / RcpScaleFactor;\n"
"			float2 b = floor((frag - 0.5f - NativeOffset) / s) * s + NativeOffset + 0.5f - frag;\n"
"			float2 un = st * WH.xy;\n"
"			float2 un0 = un + ddx(un) * b.x + ddy(un) * b.y;\n"
"			float2 g = un0 - 0.5f;\n"
"			float2 gi = floor(g);\n"
"			// Each fragment reads its own sample of the pixel's texel: its place in\n"
"			// the native pixel, whatever the texels per pixel (a minifying blur at\n"
"			// 8x otherwise ran its samples into the next texels, in stripes).\n"
"			float2 rx = max(abs(float2(ddx(un).x, ddy(un).y)) * s, float2(1.0f, 1.0f));\n"
"#if PS_PAL_FMT != 0\n"
"			// An index is a native texel's, one per texel: the texel's first sample,\n"
"			// the one a native draw writes, whatever the fragment's place; the\n"
"			// palette colours are what the taps blend.\n"
"			float2 p0 = gi * ts + 0.5f;\n"
"#else\n"
"			float2 p0 = (gi + (un - un0) / rx) * ts + 0.5f;\n"
"#endif\n"
"			dd = g - gi;\n"
"			uv = float4(p0, p0 + ts) / WH.zwzw;\n"
"#else\n"
...
"\n"
"#if PS_SAMPLE_MAP\n"
"		// The texel is the one the native-resolution pixel would read: the\n"
"		// coordinate is taken where the first fragment of that pixel sits,\n"
"		// which is where a native draw samples, so the draw picks the same\n"
"		// texels at any scale. This fragment's position inside the native\n"
"		// pixel then picks the sample inside that texel; a texture at native\n"
"		// size has only the one.\n"
"		float s = RtScaleFactor;\n"
"		float ts = 1.0f / RcpScaleFactor;\n"
"		// The native pixel is the draw's: its grid starts NativeOffset fragments\n"
"		// in, where a half-pixel offset aligned to native puts the draw.\n"
"		float2 d = floor((frag - NativeOffset) / s) * s + NativeOffset + 0.5f - frag;\n"
"		float2 st_n = uv.xy + ddx(uv.xy) * d.x + ddy(uv.xy) * d.y;\n"
"		int2 texel = int2(floor(st_n * WH.xy));\n"
"		int2 size = int2(WH.xy);\n"
"		#if PS_WMS == 0\n"
"			texel.x &= size.x - 1;\n"
"		#elif PS_WMS == 1\n"
"			texel.x = clamp(texel.x, 0, size.x - 1);\n"
"		#elif PS_WMS == 2\n"
"			texel.x = clamp(texel.x, int(MinMax.x * WH.x), int(MinMax.z * WH.x));\n"
"		#else\n"
"			texel.x = (texel.x & asint(MinMax.x)) | asint(MinMax.z);\n"
"		#endif\n"
"		#if PS_WMT == 0\n"
"			texel.y &= size.y - 1;\n"
"		#elif PS_WMT == 1\n"
...
"		#elif PS_WMT == 2\n"
"			texel.y = clamp(texel.y, int(MinMax.y * WH.y), int(MinMax.w * WH.y));\n"
"		#else\n"
"			texel.y = (texel.y & asint(MinMax.y)) | asint(MinMax.w);\n"
"		#endif\n"
"		int2 p = texel * int2(ts, ts) + int2(0.5f - d) * int2(ts, ts) / int2(s, s);\n"
"		#if PS_REGION_RECT\n"
"		p = clamp(p + int2(STRange.xy), int2(STRange.xy), int2(STRange.zw));\n"
"		#endif\n"
"#if PS_PAL_FMT != 0\n"
"		c[0] = sample_p(uint(Texture.Load(int3(p, 0)).a * 255.5f));\n"
"#else\n"
"		c[0] = Texture.Load(int3(p, 0));\n"
"#endif\n"
"#else\n"
"		uv = clamp_wrap_uv(uv);\n"
"\n"
...
"{\n"
"#if PS_FST == 0\n"
"	// The GS holds S/Q and T/Q in a signed 12-bit range; past it the\n"
"	// coordinate saturates rather than wrapping (parallel-gs does the same).\n"
"	float2 st = clamp(input.t.xy / input.t.w, -2047.0f, 2047.0f);\n"
...
"	float4 T = fetch_red(int2(input.p.xy + ChannelOffset.xy));\n"
"#elif PS_CHANNEL_FETCH == 2\n"
"	float4 T = fetch_green(int2(input.p.xy + ChannelOffset.xy));\n"
"#elif PS_CHANNEL_FETCH == 3\n"
"	float4 T = fetch_blue(int2(input.p.xy + ChannelOffset.xy));\n"
"#elif PS_CHANNEL_FETCH == 4\n"
"	float4 T = fetch_alpha(int2(input.p.xy + ChannelOffset.xy));\n"
"#elif PS_CHANNEL_FETCH == 5\n"
"	float4 T = fetch_rgb(int2(input.p.xy + ChannelOffset.xy));\n"
"#elif PS_CHANNEL_FETCH == 6\n"
"	float4 T = fetch_gXbY(int2(input.p.xy + ChannelOffset.xy));\n"
"#elif PS_DEPTH_FMT > 0\n"
"	float4 T = sample_depth(st_int, input.p.xy);\n"
"#else\n"
"	float4 T = sample_color(st, input.t.w, input.p.xy);\n"
"#endif\n"
"\n"
