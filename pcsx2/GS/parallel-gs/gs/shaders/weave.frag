...
void main()
{
    ...
    else
    {
        ...
        // The line moves if either line around it moved since the field
        // two back, or it moved itself: an edge crossing only the line
        // above or below is still motion, and weaving it would comb.
        float Mh = abs(dot(current0 - prev0, LUMA));
        float Ml = abs(dot(current1 - prev1, LUMA));
        float Mc = abs(dot(field1 - field3, LUMA));

        float diff = max(max(Mh, Ml), Mc);
