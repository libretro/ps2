...
void V_Core_AutoDMAReadBuffer(V_Core *c, int mode)
{
	...
	size = pcsx2_min_u(c->InputDataLeft, (u32)0x200);
	if (!leftbuffer)
...
{
	int i;
	u32 buff1end, buff2end;
	int cacheIdxStart, cacheIdxEnd;
	PcmCacheEntry *cacheLine, *cacheEnd;
	u32 buff1size;
	u32 TDA;
	...
	buff1end = c->ActiveTSA + pcsx2_min_u(c->ReadSize, (u32)0x100 + abs(c->DMAICounter / 4));
	buff2end = 0;
	...
	cacheIdxStart = c->ActiveTSA / pcm_WordsPerBlock;
	cacheIdxEnd   = (buff1end + pcm_WordsPerBlock - 1) / pcm_WordsPerBlock;
	cacheLine     = &pcm_cache_data[cacheIdxStart];
	cacheEnd      = &pcm_cache_data[cacheIdxEnd];
...
{
	int i;
	u32 buff1end, buff2end, buff1size, TDA;
...
void V_Core_DoDMAwrite(V_Core *c, u16* pMem, u32 size)
{
	...
	adma_enable = ((c->AutoDMACtrl & (c->Index + 1)) == (c->Index + 1));
